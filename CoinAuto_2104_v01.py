    ...
    try:
        ...
        if start_time < now < (end_time - datetime.timedelta(seconds=10)):
            ...
            if target_price < current_price : # and ma15 < current_price : 상승장일때는 이것 추가도 좋음.
                ...
                buy_money = krw * 0.2  # 잔고의 20%를 구매함.
                if krw > 5000:
                    ...
                    dbgout("BTC buy price : " + str(current_price))
                    dbgout("BTC buy coin unit : " + str(buy_coins))
                    print("BTC buy price : " + str(current_price))
                    print("BTC buy coin unit : " + str(buy_coins))
                else :
                    print("Balance is too low(under 5000won)")
                    #dbgout("Balance is too low(under 5000won)")
            else :
                print("price is not our target.")
                #dbgout("price is not our target.")
            ...
            if (sell_coins*current_price) < 5000:
                ...
                if sell_coins<0.0001: # 비트코인 주문 최소한도 미만이라서 중단
                    ...
                    dbgout("BTC sell price : " + str(current_price))
                    dbgout("BTC sell coin unit : " + str(sell_coins))
                    print("BTC sell price : " + str(current_price))
                    print("BTC sell coin unit : " + str(sell_coins))
        time.sleep(60)  # 1분마다 확인해서 구매여부 판단.
